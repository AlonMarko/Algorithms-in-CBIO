...
def transition_emission_creator(initial_emission, p, q):
    """
    ...
    :param initial_emission: the initial emission table
    :return:
    """
    motif_len = len(initial_emission)
    emissions_table = np.r_[[np.zeros(4)], [np.full(4, 0.25)], initial_emission, [np.full(4, 0.25)], [np.zeros(4)]]
    emissions_table = np.c_[emissions_table, np.zeros(motif_len + 4), np.zeros(motif_len + 4)]
    emissions_table[0][5] = 1
    emissions_table[motif_len + 3][4] = 1
    transition_table = create_transition_table(p, q, motif_len)
...
def create_transition_table(p, q, motif_len):
    """
    :param p:
    :param q:
    :param motif_len:
    :return:
    """
    T = np.zeros((motif_len + 4, motif_len + 4))
    T[0][1] = q
    T[0][motif_len + 2] = 1 - q
    T[1][1] = 1 - p
    T[1][2] = p
    for i in range(1, motif_len + 1):
        T[i + 1][i + 2] = 1
    T[motif_len + 2][motif_len + 2] = 1 - p
    T[motif_len + 2][motif_len + 3] = p
    T[motif_len + 3][motif_len + 3] = 1
    return T
